...
val exists
  :  (abs, _) t
  -> [ `Yes | `Yes_modulo_links | `Yes_as_other_object | `Unknown | `No ] Deferred.t
...
(** [find item path] searches for [item] in [path], returning the full
    ...
    found. *)
val find_item : (rel, 'typ) item -> abs_dir list -> (abs, 'typ) t option Deferred.t

val reify : (abs, dir) t -> (abs, dir) t Deferred.Or_error.t
...
val iter
  :  (abs, dir) t
  -> f:
       ([ `File of (rel, file) t | `Dir of (rel, dir) t | `Broken_link of (rel, link) t ]
        -> unit Deferred.t)
