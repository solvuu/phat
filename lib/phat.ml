...
let rec rel_dir_item_of_sexp sexp : (rel, dir) item =
  match sexp with
  | Sexp.List [ Sexp.Atom "Dir"; name ] -> Dir (name_of_sexp name)
  | Sexp.List [ Sexp.Atom "Link"; name; x ] ->
    let name = name_of_sexp name in
    map_any_kind (dir_of_any_kind_of_sexp x) { map = (fun p -> Link (name, p)) }
  | Sexp.Atom "Dot" -> Dot
  | Sexp.Atom "Dotdot" -> Dotdot
  | _ -> failwithf "invalid sexp for (rel,dir) item: %s" (Sexp.to_string sexp) ()

and abs_dir_item_of_sexp sexp : (abs, dir) item =
  match sexp with
  | Sexp.Atom "Root" -> Root
  | _ -> failwithf "invalid sexp for (abs,dir) item: %s" (Sexp.to_string sexp) ()

and rel_file_item_of_sexp sexp : (rel, file) item =
  match sexp with
  | Sexp.List [ Sexp.Atom "File"; name ] -> File (name_of_sexp name)
  | Sexp.List [ Sexp.Atom "Link"; name; x ] ->
    let name = name_of_sexp name in
    map_any_kind (file_of_any_kind_of_sexp x) { map = (fun p -> Link (name, p)) }
  | _ -> failwithf "invalid sexp for (rel,file) item: %s" (Sexp.to_string sexp) ()

and rel_file_of_sexp sexp : rel_file =
  match sexp with
  | Sexp.List [ Sexp.Atom "Item"; item ] -> Item (rel_file_item_of_sexp item)
  | Sexp.List [ Sexp.Atom "Cons"; item; x ] ->
    Cons (rel_dir_item_of_sexp item, rel_file_of_sexp x)
  | _ -> failwithf "invalid sexp for (rel,file) t: %s" (Sexp.to_string sexp) ()

and abs_file_of_sexp sexp : abs_file =
  match sexp with
  | Sexp.List [ Sexp.Atom "Cons"; item; x ] ->
    Cons (abs_dir_item_of_sexp item, rel_file_of_sexp x)
  | _ -> failwithf "invalid sexp for (abs,file) t: %s" (Sexp.to_string sexp) ()

and rel_dir_of_sexp sexp : (rel, dir) t =
  match sexp with
  | Sexp.List [ Sexp.Atom "Item"; item ] -> Item (rel_dir_item_of_sexp item)
  | Sexp.List [ Sexp.Atom "Cons"; item; x ] ->
    Cons (rel_dir_item_of_sexp item, rel_dir_of_sexp x)
  | _ -> failwithf "invalid sexp for (rel,dir) t: %s" (Sexp.to_string sexp) ()

and abs_dir_of_sexp sexp : abs_dir =
  match sexp with
  | Sexp.List [ Sexp.Atom "Item"; item ] -> Item (abs_dir_item_of_sexp item)
  | Sexp.List [ Sexp.Atom "Cons"; item; x ] ->
    Cons (abs_dir_item_of_sexp item, rel_dir_of_sexp x)
  | _ -> failwithf "invalid sexp for (abs,dir) t: %s" (Sexp.to_string sexp) ()

and dir_of_any_kind_of_sexp sexp : dir of_any_kind =
  try `Rel (rel_dir_of_sexp sexp) with
  | Failure _ -> `Abs (abs_dir_of_sexp sexp)

and file_of_any_kind_of_sexp sexp : file of_any_kind =
  try `Rel (rel_file_of_sexp sexp) with
  | Failure _ -> `Abs (abs_file_of_sexp sexp)
;;
...
let name s =
  if String.mem s '/'
  then errorh [%here] "slash not allowed in file or directory name" s sexp_of_string
  else if String.equal s "." || String.equal s ".." || String.equal s ""
  then errorh [%here] "invalid file or directory name" s sexp_of_string
  else Ok s
;;
...
let rec is_normalized : type a b. (a, b) t -> bool = function
  | Item _ -> true
  | p -> is_normalized_no_dot p

and is_normalized_no_dot : type a b. (a, b) t -> bool = function
  | Item Dot -> false
  | Cons (Dot, _) -> false
  | Item _ -> true
...
and is_normalized_no_dot_or_dotdot : type a b. (a, b) t -> bool = function
  | Item Dot -> false
  | Item Dotdot -> false
  | Cons (Dot, _) -> false
  | Cons (Dotdot, _) -> false
...
let rec has_link : type a b. (a, b) t -> bool = function
  | Item (Link _) -> true
  | Cons (Link _, _) -> true
  | Cons (_, p) -> has_link p
  | _ -> false
...
let kind_of : type a b. (a, b) t -> b of_any_kind =
 fun p ->
  match p with
  | Item Root -> `Abs p
  | Item (File _) -> `Rel p
...
let rec obj_aux : type k k' b. (k, b) t -> (k', b) t -> k of_any_typ =
 fun p suffix_p ->
  match suffix_p with
  | Item (File _) -> `File p
...
let typ_of : type k. (k, _) t -> k of_any_typ = fun p -> obj_aux p p

(******************************************************************************)
(* Operators                                                                  *)
(******************************************************************************)

let rec concat : type a b. (a, dir) t -> (rel, b) t -> (a, b) t =
 fun x y ->
  match x with
  | Item x -> Cons (x, y)
...
let rec resolve_any_kind : type a b. (a, b) t -> b of_any_kind = function
...
and resolve_item : type a b. (a, b) item -> b of_any_kind =
 fun x ->
  match x with
  | Root -> `Abs (Item x)
  | File _ -> `Rel (Item x)
...
and resolve : type b. (abs, b) t -> (abs, b) t = function
  | Item Root as x -> x
  | Cons ((Root as x), path) -> (
...
and normalize_item : type a b. (a, b) item -> (a, b) item =
 fun item ->
  match item with
  | File _ -> item
  | Dir _ -> item
...
let compare : type a b. (a, b) t -> (a, b) t -> int =
...
module Make_relative = struct
  let rec skip_common_prefix
   ...
   fun p1 p2 ->
    match p1, p2 with
    | Item i1, Item i2 -> if Poly.( = ) i1 i2 then None, None else Some p1, Some p2
    | Item i1, Cons (i2, q2) ->
 ...
 ;;
  ...
  let rec backwards : type k. (k, dir) t -> (rel, dir) t option =
    ...
    | Cons (Dir _, p_rel) -> backwards p_rel >>| fun q -> Cons (Dotdot, q)
    | Cons (Link _, p_rel) -> backwards p_rel >>| fun q -> Cons (Dotdot, q)
    | Cons (Dot, p_rel) -> backwards p_rel
    | Cons (Dotdot, _) -> None
  ;;

  let rec last_item : type o. (rel, o) t -> (rel, o) t = function
    | Item _ as p -> p
  ...
  let rec ty : type k o. (k, o) t -> o ty = function
    | Item i -> item_ty i
    | Cons (_, p) -> ty p

  and item_ty : type k o. (k, o) item -> o ty = function
    | Root -> TyDir
    | Dir _ -> TyDir
    | Dot -> TyDir
    | Dotdot -> TyDir
  ...
  let make_relative : type o. (abs, o) t -> from:(abs, dir) t -> (rel, o) t =
   fun p ~from:origin ->
    ...
    | Cons (Root, q_rel), Cons (Root, p_rel) ->
      let rec k : type o. o ty -> (rel, o) t -> (rel, o) t =
       fun p_rel_ty p_rel ->
        match p_rel_ty with
        | TyDir -> (
          match skip_common_prefix q_rel p_rel with
          | None, None -> Item Dot
          | Some q_rel, None -> Option.value_exn (backwards q_rel)
...
module Elem : sig
  ...
  let elems = function
    ...
    | s ->
      ...
      >>| fun l -> if leading_slash then "/" :: l else l
  ;;
  ...
  let rel_dir_of_elems elems : rel_dir Or_error.t =
    ...
    | [] -> assert false
    | "/" :: _ ->
      errorh [%here] "relative path cannot begin with root directory" elems sexp_of_elems
    | _ :: tl ->
      if List.mem tl "/" ~equal:String.equal
      then
        errorh
          [%here]
          "root directory can only occur as first item in path"
          elems
          sexp_of_elems
      else (
        let item = function
    ...
    | "/" :: tl ->
      ...
      then
        errorh
          ...
          elems
          sexp_of_elems
      else rel_dir_of_elems tl >>| fun reldir -> Cons (Root, reldir)
    | _ ->
      errorh [%here] "absolute path must begin with root directory" elems sexp_of_elems
  ;;
  ...
  let rel_file_of_elems elems : rel_file Or_error.t =
    ...
    | _ -> (
      ...
      match last with
      | "." | "" | ".." ->
        errorh [%here] "path cannot be treated as file" elems sexp_of_elems
      | _ -> (
        match elems with
        | [] -> Ok (Item (File last))
        | _ -> rel_dir_of_elems elems >>| fun dir -> concat dir (Item (File last))))
  ;;
  ...
  let abs_file_of_elems elems : abs_file Or_error.t =
    ...
    | [] -> assert false
    | [ "/" ] ->
      errorh [%here] "root directory cannot be treated as file" elems sexp_of_elems
    | "/" :: rest -> (
      ...
      | "." | "" | ".." ->
        errorh [%here] "path cannot be treated as file" elems sexp_of_elems
      | _ -> abs_dir_of_elems ("/" :: rest) >>| fun dir -> concat dir (Item (File last)))
    | _ ->
      errorh [%here] "absolute path must begin with root directory" elems sexp_of_elems
  ;;
end
...
let rec last_item : type o. (rel, o) t -> (rel, o) item = function
  | Item i -> i
