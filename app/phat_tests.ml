...
let ( >::= ) test_name (f : test_ctxt -> unit Deferred.t) : test =
  test_name
  >:: fun test_ctxt ->
  try Thread_safe.block_on_async_exn (fun () -> f test_ctxt) with
...
and update_level_item
 ...
 fun ~root i -> function
  ...
  | Phat.Link (_, p) -> (
    ...
    | `Abs p -> update_level ~root i p - update_level ~root:Phat.root 0 root
...
let new_name =
  ...
  fun () ->
    incr k;
    "foo" ^ string_of_int !k |> Phat.name |> ok_exn
;;
...
module NR = struct
  let rec rel_dir_item ?(no_dot = false) ~link_level ~root ~level () =
    ...
    if Random.bool ()
    then Phat.Dir (new_name ())
    else if Random.bool () && not no_link
    then
      Phat.map_any_kind
        (dir_path ~link_level:(link_level - 1) ~root ~level ())
        { Phat.map = (fun p -> Phat.Link (new_name (), p)) }
    else if (not bottom) && (not no_dot) && Random.bool ()
    then Phat.Dotdot
...
module R = struct
  let rec rel_dir_item ~link_level ~root ~level () =
    ...
    then
      Phat.map_any_kind
        (dir_path ~link_level:(link_level - 1) ~root ~level ())
        { Phat.map = (fun p -> Phat.Link (new_name (), p)) }
    else if Random.bool () && not bottom
    then Phat.Dotdot
    else Phat.Dot
  ...
  and path_resolving_to ~link_level ~root ~level p () =
    ...
    if Float.( < ) (Random.float 1.) 0.1
    then `Rel (Phat.Item link)
    else (
...
let normalization_is_idempotent _ =
  ...
  for _ = 1 to 1000 do
    Phat.map_any_kind
      (R.dir_path ~link_level:4 ~root:Phat.root ~level:0 ())
      { Phat.map = check }
  done
;;
...
let resolution_for_eventually_abs_paths _ =
  let failure p_ref p p_res =
    ...
    assert_failure msg
  in
  let check p_ref p p_res = if Poly.( <> ) p_ref p_res then failure p_ref p p_res in
  for _ = 1 to 1000 do
...
let resolution_is_identity_for_paths_without_links _ =
  let failure dir dir' =
    ...
    assert_failure msg
  in
  let check dir dir' = if Poly.( <> ) dir dir' then failure dir dir' in
  for _ = 1 to 1000 do
...
let resolution_eliminates_links _ =
  ...
  let check p =
    Phat.map_any_kind
      (Phat.resolve_any_kind p)
      { Phat.map = (fun p_res -> check_aux p p_res) }
  in
  for _ = 1 to 1000 do
    Phat.map_any_kind
      (R.dir_path ~link_level:4 ~root:Phat.root ~level:0 ())
...
let create_test_directory path =
  ...
  >>= fun () ->
  Writer.save (f "foo/bar/baz") ~contents:"baz"
  >>= fun () ->
  Unix.symlink ~link_name:"foo/bar/baz" ~target:(f "qux")
  >>= fun () ->
  Unix.symlink ~link_name:".." ~target:(f "foo/bar/norf")
  >>= fun () -> Unix.symlink ~link_name:"foo/bar/booz" ~target:(f "broken")
;;
...
let test_directory_description =
  ...
  let broken = broken_link_exn "broken" [ "foo"; "bar"; "booz" ] in
...
let filesys_exists ctx =
  ...
  let check : type o. (Phat.rel, o) Phat.t -> unit Deferred.t =
   fun p ->
    ...
    >>| function
    | `Yes -> ()
    | `Yes_modulo_links | `Yes_as_other_object | `Unknown | `No ->
      let msg =
        sprintf "Failed to detect path %s" (Sexp.to_string_hum (Phat.sexp_of_t p))
      in
      assert_failure msg
  in
...
let fold_works_on_test_directory ctx =
  ...
  >>= fun () ->
  Phat.fold tmpdir ~init:[] ~f:(fun accu elt -> return (pw_of_elt elt :: accu))
  >>| function
  | Ok l ->
    assert_equal
      ~printer:(List.to_string ~f:to_string)
      expected
      (List.sort ~compare:Poly.compare l)
...
let fold_follows_links_works_on_test_directory ctx =
  ...
  | Ok l ->
    assert_equal
      ~printer:(fun xs -> String.concat ~sep:"\n" (List.map xs ~f:to_string))
      expected
      (List.sort ~compare:Poly.compare l)
  | Error _ -> assert_failure "Fold failed on test directory"
;;
...
let suite =
  ...
  >::: [ "Name constructor" >:: name_constructor
       ...
       ; "Resolution is the identity for paths without links"
         >:: resolution_is_identity_for_paths_without_links
       ...
       ; "Fold (when following link) works on test dir"
         >::= fold_follows_links_works_on_test_directory
       ]
;;
